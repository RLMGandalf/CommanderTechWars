...
$(document).ready(function () {
    ...
    function PlanetsViewModel() {
        ...
        self.systemName = ko.computed(function() { return self.state().system || ''; });
        ...
        self.sun = ko.computed(function() {
            return _.find(self.celestialViewModels(), { isSun: true }) || { isSelected: false };
        });
        ...
        self.planetToolTip = function (planet, threshold) {
            ...
            return '<div class="div_planet_list_item_center">' +
                        '<div class="planet_title"> ' + (planet.isSun ? self.systemName() : planet.name) + ' </div>' +
        ...
        self.handleClick = function(index) {
            if (!self.celestialControl())
                api.Panel.message(api.Panel.parentId, 'planets.click', index);
            else if (self.targeting() && self.alivePlanets()[index].isValidTarget)
