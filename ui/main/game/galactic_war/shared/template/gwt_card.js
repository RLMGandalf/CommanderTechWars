...
globals.CardViewModel = function (params) {
    ...
    self.locDesc = ko.computed(function () {
        if (_.isArray(self.desc()))
            return _.reduce(self.desc(), function (sum, val) { return loc(sum) + loc(val); });
    ...
    var loadCard = function(card, data) {
        if (_.isEmpty(card)) {
            self.desc('Data Bank holds one Tech. Explore systems to find new Tech.');
            self.summary('Empty Data Bank');
