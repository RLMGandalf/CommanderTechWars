    ...
    function SocialViewModel() {
        ...
        self.friends = ko.computed(function () {
            return _.filter(self.usersSortedByLastInteractionTime(),
                            function (element) { return element.friend() && element.uberId() !== model.uberId() });
        });
        ...
        self.readyToSendLobbyInfo.subscribe(function (value) {
            ...
            _.forEach(self.confirmedInvites(), function (element) {
                if (available) {
                    jabber.sendCommand(element, 'game_lobby_info', new_info);
        ...
        self.findUserId = function () {
            engine.asyncCall('ubernet.call', '/GameClient/UserId?UberName=' + self.contactSearch(), false)
                    .done(function (data) {
                        var result = JSON.parse(data);
        ...
        self.finishChangeDisplayName = function () {
            ...
            engine.asyncCall('ubernet.call', '/GameClient/UpdateUserTitleDisplayName?DisplayName=' + self.newDisplayName(), true)
                   .done(function (data) {
                       var result = JSON.parse(data);
            ...
            switch (type) {
                ...
                case 'game_lobby_info':
                    if (model.acceptedGameInviteFrom() === uberid) {
                        ...
                        api.Panel.message('game', 'join_lobby', command.payload);
                    }
                    break;
            }
        }
